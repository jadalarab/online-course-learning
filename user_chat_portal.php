		...
		<div class="row">
            <div class="col-lg-12 grid-margin">
              <div class="card">
				...
				<?php $chat_room_id = $conn->query("select * from chat_room where student_id='$student_id'");
					$count_id=0;
					$chat_room_id_found="";
					foreach($chat_room_id as $id){
					$chat_room_id_found = $id[0];
					$count_id++;
					} ?>
					<input type="hidden" value=<?=$chat_room_id_found?> id="room_id"/>
													<input type="hidden" value="<?=$userPicture?>" id="path"/>

					<?php 
					if($count_id == 0){ ?>
				<button type="button" id="open_chat_room" 
                     class="btn btn-success btn-fw">Open Chat room</button>
			<?php } else { $seen_user=$conn->query("update chat_messages set seen_by_user=1 where chat_room_id='$chat_room_id_found'");
			?>
